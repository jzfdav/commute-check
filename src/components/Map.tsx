...
import type { RouteData, Location } from '../types';
...
export function Map({ originA, originB, destA, destB, routeA, routeB, mode }: MapProps) {
  const polylineA = useMemo(() => routeA ? decodePolyline(routeA.geometry) : [], [routeA]);
  const polylineB = useMemo(() => routeB ? decodePolyline(routeB.geometry) : [], [routeB]);

  const bounds = useMemo(() => {
    const points = [
      [originA.lat, originA.lng],
      [destA.lat, destA.lng],
      [mode === 'destinations' ? destB.lat : originB.lat, mode === 'destinations' ? destB.lng : originB.lng],
    ].filter(p => p[0] !== 0) as [number, number][];

    return points.length > 0 ? L.latLngBounds(points) : L.latLngBounds([[12.97, 77.59]]);
    ...
    <div style={{ height: '400px', width: '100%', borderRadius: '8px', overflow: 'hidden' }}>
      ...
      >
        ...
        <Marker position={[destA.lat, destA.lng]}>
          <Popup>{mode === 'destinations' ? 'Option A End' : 'Destination'}</Popup>
        </Marker>
